...
|---|---|---|---|
| **PWA quiz tab** | `index.html` (play loop) | `players/{name}`; read-only cross-player: family boards (English profiles only, `ruTrack` docs excluded), Artem's Nicole-RU program admin (`players/nicole_ru` + exercises) | `players/{name}` (qStats, catStats, lvlStats, recentSessions, totals, streaks, daily_activity) |

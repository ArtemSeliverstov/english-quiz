...
**Last refresh**: 2026-07-08 — stats-review (regen+audit half, no fresh quiz data). Folded 07-07 PV cold-production batch via `pv_cold_streak.js`: get_around_to still 🟡 regressing (0/3, 1/3 correct — fossil holds #1); get_through 1st cold win (1/0, 7/07 → 🟢→🏆 building 1/3); come_up_with / turn_out / go_about each 1 cold win (building); cut_down_on 1/2 (`cut_down_on_particle_drop` count 2 cross-session — stays PV-lane per doctrine, NOT promoted to weak_patterns); bring_in / read_up_on ⚪ unstable 0/1. **Top-5 queue ordering unchanged** (get around to → get through¹ → come up with → read up on → cut down on). Quiz-derived Status still reflects `qStats` as of 2026-05-20; full Status re-derivation awaits fresh quiz data. Prior — 2026-07-04: Top-5 drill queue block added (T3). 2026-07-03: production-tier data folded.

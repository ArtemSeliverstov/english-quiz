...
Older RTDB-era sync bugs (question migration, pull-before-push race, bulk-fetch truncation, migration whitelist) → `references/archive/bug-log-pre-2026-05.md`. Their live rules are in the summary below (#5, #6).
...
- `ee25` (Natural English, B1, input): keyed `ans:'get going|be off|head off|crack on'` rejected *be going*, but "I'd better be going!" is a textbook 2-word casual British leave-taking. Hit two players (artem 0/2, anna 1/3).
...
Obsolete deploy.html/GitHub-Trees pipeline bugs (sequential PUT, Python f-string, triple paren, hardcoded display version) → `references/archive/bug-log-pre-2026-05.md`.

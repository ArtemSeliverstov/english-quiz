...
| `register_rubric` | map | Coach tab (PWA) `coachWriteSessionLogStandalone` | stats-review (active, 2026-05-13), mistakes-review (future) | Silent register-fluency measurement (Free Write only, as of v20260512-r3). `{chunk_density: 1..5, register_match: 1..5, calque_count: int, discourse_marker_variety: int, confidence: 'high'\|'low'}`. Aggregated into per-player rolling means + trend flags by daily `stats-review`. Full anchors + flag rules + aggregation procedure in `references/register-rubric.md`. Never shown to the learner. |

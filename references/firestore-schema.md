...
| `study_plan` | map | CC only (Firebase MCP) | PWA learner home card, Artem program dashboard, stats-review | RU-track program plan: `start_date`/`end_date`, `daily_target_items`, `weeks[]` {n, start, end, topics, kickoff/dictation/mock}. PWA reads but never echoes it back — excluded from `buildPayload` so a stale local copy can't clobber CC edits. `plans/ru-track-nicole.md`. |
| `daily_activity` | map | PWA play loop (`bumpDailyActivity`) | program dashboards, stats-review | `{"YYYY-MM-DD": {items, correct}}` per-day adherence record, all players. Durable beyond `recentSessions` FIFO-10. Date key follows the `lastPlayedDate` UTC convention. |
...
| `register_rubric` | map | Coach tab (PWA) `coachWriteSessionLogStandalone`; `log_coach_session.js` (CC free-write, REQUIRED since 2026-07-04 — CR1) | stats-review (active, 2026-05-13), mistakes-review (future) | Silent register-fluency measurement on Free Write sessions. `{chunk_density: 1..5, register_match: 1..5, calque_count: int, discourse_marker_variety: int, confidence: 'high'\|'low'}`. Aggregated into per-player rolling means + trend flags by `stats-review`. Full anchors + flag rules + aggregation procedure in `references/register-rubric.md`. Never shown to the learner. |
| `interview_rubric` | map | `/interview-prep` CC skill via `log_coach_session.js` (ip_* sessions) | stats-review (active, 2026-07-04 — T5) | Silent interview-delivery measurement. `{structure_score: 1..5, specificity_score: 1..5, confidence_balance: 1..5, avg_wpm, avg_filler_density_per_min, avg_pause_ratio, confidence, rubric_version: 1}`. Aggregation per `references/interview-rubric.md` § Stats-review aggregation. Never shown to the learner. |

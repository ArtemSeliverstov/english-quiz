...
Possible values:
- `claude_code` — Claude Code via `tools/update_coach_notes.js` or Firebase MCP (remote-CC sessions)
- `coach_pwa` — the PWA Coach tab via Firestore JS client
...
- Auto-writing repo-file edits (CLAUDE.md, references/, progress/) — two mechanical exceptions: tracker markdown regenerated by stats-review, and `tools/mistake_verdicts.json` ledger appends by mistakes-review/quiz-development
...
For the PWA Coach tab, same flow via Firestore JS client.

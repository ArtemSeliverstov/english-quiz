...
## 2026-09-23
### v20260923 — RU: Орфография (30) — автоматизмы свободного письма Эрнеста

- **Источник**: Артём прислал образцы живого письма Эрнеста (переписка 23.09, Эрнесту не раскрывается). Сорвались: `-тся/-ться` — **дважды в одном сообщении** (нравиться, кажеться), дефис (что то), слитное наречие (в кратце). Зоны, где на CC-сессиях он даёт 9/9: правило знает, автоматизма нет. Под эссе гос. аттестации (май-июнь 2027) этот фронт весит больше тонкостей разбора СПП.
- **Новая категория `RU: Орфография`**, `ru_or01-30`, общая с Никой (B1 — её уровень, B2 — ловушки). Три детектора: «ь в вопросе = ь в слове» · «кое-/-то/-либо/-нибудь на дефисе» · «нет зависимого слова → наречие слитно». Ловушки: после нужно/пора/может — начальная форма; «кое с кем» в три слова; вначале vs в начале книги; наверх vs на верх горы.
- Типы: 14 gap · 10 input (33%, норма ≥20%) · 3 mcq · 3 error_correction. Строка `CAT_STAKES_ERNEST` для категории.
- **Окно не тронуто** (решение Артёма): `active_categories` остаётся Сложное + Н/НН + Морфемика, орфо-блок ждёт закрытия Н/НН. До ротации фронты идут только разминками CC-сессий.
- **Правка очереди авторинга** в `plans/ru-track-ernest.md`: Орфо вынесено вперёд Сложного и Обособлений. Хвосты и решение — в `progress/ru-program-tracker-ernest.md` § «Орфо-фронты из живого письма».

Q count: 2465 (+30) · Version: v20260923

---

## 2026-09-20
### v20260920 — Закрытие категорий как событие + школьные ставки (ernest_ru)

- **Мотивация подростка** (запрос Артёма 20.09, доктрина §5 — «rare meaningful achievements» и «ставки, не спарклы»; §6 запреты не тронуты):
- **Закрытие категории — событие**: одноразовая модалка «Тема закрыта!» (переиспользован `#grad-modal`: 🏁, конфетти, кнопка «Класс!») при появлении новых записей в `learning_path.mastered_categories`; постоянная полка «Закрытые темы» (чипы) на learner-home между медалями и стриками. Гард `DB._masteredCelebrated` локальный, переживает мердж `loadFromFirebase`; на свежем устройстве празднуются только закрытия ≤14 дней (`mastered_date`), старые базлайнятся молча. RU/EN тексты по `isRuTrack`.
- **Школьные ставки**: `CAT_STAKES_ERNEST` — одна строка 🎯 на первом вопросе категории за сессию (5 RU-категорий, привязка к аттестации Tamos 10 класс), только `ernest_ru`. CSS `.cat-stakes` поверх `.multi-intro`.
- **Фикс данных**: `mastered_categories` у ernest_ru переведены из строк в объекты `{category, mastered_date}` — строки молча выпадали из 30-дневного spaced-review в `applyLearnerWindowFilter` (движок читает `m.category`).
- **Попутно**: `spawnGradConfetti()` выделен из `showGradModal` (переиспользование); `#grad-later-btn` получил id (модалка mastery его прячет); `.claude/launch.json` со статик-сервером для превью.
- Проверено вживую (localhost, профиль ernest_ru): модалка один раз, повторный заход — только полка; 🎯-строка на первом вопросе категории; консоль чистая. Записи: `design-decisions.md` §UX ×2.

---

## 2026-09-20
### ernest_ru: ротация окна learning_path (решение Артёма 20.09)

- **Данные**: квиз 19–20.09 — Синтаксис 10/10, Причастия 10/10 за два дня (переросли окно), Сложное предложение 4/5 → 2/5. Промахи — ровно хвост S8: `ru_sl12` виды придаточных 0/3 (паттерн `slozh_klassif` из диагностики 18.08, вопрос залочен спейсингом до 01.11), `ru_sl09` БСП двоеточие вместо тире, `ru_sl14` косвенная речь. Дрилл без энкодинга = фрустрация; критика Артёма («не самая мотивирующая тема к возвращению») подтверждена данными.
- **PATCH `players/ernest_ru.learning_path`**: Синтаксис + Причастия → `mastered_categories`; в окно — «RU: Н/НН» (33 вопроса) и «RU: Морфемика и части речи» (36); Сложное предложение остаётся; `next_unlock_options` пуст.
- **Следующий шаг**: короткая CC-сессия ~20 мин по видам придаточных (детектор «задай вопрос от главного слова») + БСП-знаки — до того, как квиз снова упрётся в эту категорию.

---

## 2026-09-20
### RU-сессия Ники: разминка 3/5 холодных + детектор-дрил «пол-» (3 режима) и «по-»

- **Разминка (5 ПОРА из очереди)**: холодные попадания — **недалеко** (синоним, `ne_slitno_synonym_exists` 1/3) · **запятая при одном герое** (`comma_before_i_one_subject` 1/3, детектор сформулировала сама: «один герой») · **нелепый** (`ne_inseparable_split` 1/3). Промахи: **пол чайной ложки → слитно** (2-й промах подрегулы «прилагательное вклинилось»; немедленный ретест «пол зелёного яблока» ✅) · **по зимнему лесу → слитно** (3-й промах `po_adjective_hyphen_overapplied`).
- **Дрил «детектор → ответ»**: контрастная пара по-осеннему/по осеннему **инвертирована целиком**, «по доброму городу» → дефис; взяла только после пошагового прогона «найди существительное». Правило у неё есть — детектор срабатывает наоборот; на ретесте 09-23 давать пару, не одиночный пример. Новый паттерн: **пол-арбуза → слитно** (`pol_hyphen_before_vowel_missed`, первое предъявление режима «Л/гласная/большая буква»); после разбора тройка полкруга/пол-апельсина/пол холодного ✅✅✅. Финал на подъёме: по-хитрому, по новой дороге, неуклюжий — все с детекторами.
- **Лог**: error_correction `1789910434471` (17 items, 11/17, 3 error_types, 3 cold_passes). Очередь перегенерирована: 23 открытых, ПОРА осталось 2 (`nn_vetreniy_exception`, `po_prefix_vs_preposition_noun` — в разминку не влезли, 09-22 подойдёт волна из 8). Заметка: `errors[]` в строку писать **без** префикса даты — `ru_retest_queue.js` добавляет её сам, а exercises write-once (в «Что было» сегодня «09-20 09-20», косметика). Письмо: «дефиз» ×2 → инвентарь, новых позиций нет.

## 2026-09-19
### RU-сессия Ники: мини-тест 6 кл 13/15 (школьный тест — 9/21) + вердикт пробника квиза

- **Контекст**: на неделе школьный тест по темам 6 класса — **9/21** (сначала прозвучало «7 из 25», Ника поправила 19.09; в meta.context строки Firestore осталось 7/25 — exercises write-once, править нельзя, каноничная цифра здесь). Домашняя сессия по протоколу: холодная разминка из очереди (5 паттернов) → мини-тест 15 вопросов по кластерам вступительного → разбор. **13/15** — разрыв со школьным указывает на запуск проверок под стрессом / формат теста, не на знания. Ждём фото школьного теста с ошибками. Лог: mock `1789813080360` (15 items, 6 error_types, 9 cold_passes).
- **Разминка (холодные промахи)**: по-новому слитно (5-й, `po_hyphen_adverb_missed`) · не лёгкая, а трудная слитно (4-й а-контраст) · недорогой, но красивый раздельно (гиперкоррекция «но» повторно) · ветреная нн + фейк-правило «после е две нн». Попадания: глиняная -ЯН- с детектором.
- **Мини-тест промахи (2)**: вводное «к счастью» → «запятые не нужны» — в разборе 3 раза подряд ставила одну запятую из двух, вторая скобка не автоматизирована; взяла после приёма [скобки]→запятые. «Кто-то» → раздельно (новый паттерн `hyphen_to_suffix_missed`) — при этом «кое о чём»/«кое у кого» держит уверенно; стишок «-то, -либо, -нибудь, кое- — дефис не забудь».
- **Очередь ретестов** перегенерирована: 22 открытых, 9 паттернов получили холодное попадание 1/3 (числительные, кое+предлог, наречие/прил, сущ-действие, ъ, запятая перед «и», однородные, стык-Н, -ЯН-).
- **Вердикт пробника квиза** (решение 08.09, срок 15.09): `daily_activity` в `players/nicole_ru` пуст — 0 открытий за 08–19.09. По правилу: **спейсинг остаётся в CC, в квиз-контент для nicole_ru больше не вкладываемся**; W2–W5 категории не авторим.

Q count: 2435 (без изменений)
### v20260911 — RU: Математические термины ×20 (`ru_mt17–36`) — лексика условий олимпиады + правила юниорской в плане Ники

- **Контекст**: школьный этап юниорской олимпиады в TAMOS (сентябрь) — **7/20** у Ники, задачи лёгкие, лимит времени жёсткий; результат лёг в коридор прогноза плана (~8–12/30 при темпе 6–7,5 мин/задача). Firestore: `players/nicole_ru` — категория «RU: Математические термины» (16 вопросов с 03.08) ни разу не открыта, квиз не открывался с 28.07; mathsprint 02–10.09 — Лайт/База чисто и быстро, Порядок 3 ошибки, Дроби не играны с 28.07.
- **Партия 2 терминов**: 20 вопросов по частотности слов в реальных районных вариантах 7 кл (matol.kz 1186/1192, по 30 задач): натуральное число (её собственный пропуск — «не знала, что такое натуральные») · простое/составное · делитель/кратное/делится · сумма цифр · последовательные · двузначные · степень/показатель · квадрат/куб/полный квадрат · среднее арифметическое · наибольшее · инструкции «вычислите/сравните». Типы: input 7 · gap 5 · mcq 8 (распознавание по числовому ответу, как `ru_mt12`). Intro-карточки на первом вопросе каждой семьи, примеры в intro не совпадают с условиями. Четыре вопроса — прямые реплики задач вариантов (24/25 №4, №9, №10, №21). Категория уже в `nicole_ru.learning_path.active_categories` — PATCH не нужен.
- **План** (`plans/math-track-nicole.md`): правила РНПЦ «Дарын» 2024 прочитаны со скана — 4 этапа (школьный → районный → областной → респ.), районный = 30 закрытых тестов / 90 мин / 1–3 балла по уровню / макс. 50, призёры ≤45%, апелляций нет; на районный идут победители школьного (п. 13). Чекпойнты пересчитаны на регламент 3 мин/задача; мини-прогоны возобновляются с недели 14–20.09; учебники класса 7Е (Алгебра 7, Геометрия 7 — Мектеп 2025) с порядком тем → дорожка A. Новый лог `diagnostics/nicole_math_session-log_2026-09.md` — запись о школьном этапе. Терм-инвентарь — статус канала 2.

Q count: 2435 (Δ+20) · Version: v20260911

---

## 2026-09-08
### v20260908 — RU: НЕ с частями речи ×15 (`ru_ne01–15`) + очередь ретестов + протокол RU-сессий

- **Пробник квиза для Ники 08–15.09** (решение по итогам mini-mock B 07.09: правила знает, проверку в холодном режиме не запускает; профиль `nicole_ru` в квизе не открывался ни разу — 0 ответов). Новая категория «RU: НЕ с частями речи», 15 вопросов под её детекторы: НЕ с глаголом раздельно (её «незнаю» из письма) · без НЕ не живёт → слитно (нездоровится, нелепый, ненавидит, негодовать) · замена синонимом → слитно · спор «, а» → раздельно (3-й промах а-контраста) · «но» не спорит (её гиперкоррекция 07.09). Типы: input 4 · gap 8 · mcq 2 (варианты «написание + причина» = проверка + ответ) · error_correction 1. Карточки `intro` на первом вопросе каждого подправила; hint у input = вопрос-проверка. Категория добавлена в `nicole_ru.learning_path.active_categories` (PATCH, остальные поля целы). Решение 15.09 по `daily_activity`: ≥3 дня → авторим W2–W5, иначе спейсинг остаётся в CC.
- **`tools/ru_retest_queue.js`** — генератор `progress/ru-retest-queue-nicole.md` из строк `exercises` (generated view): паттерн ретестируется холодно через +3 дня при 0 холодных попаданий, +7 при одном, +14 при двух, закрывается после трёх; попадания логируются как `meta.cold_passes: {pattern_id: n}`, промах сбрасывает счёт. Сейчас 21 открытый паттерн, 14 со статусом ПОРА (история до 08.09 без cold_passes). Зарегистрирован в `tools/README.md` + CLAUDE.md (трим до 512 слов).
- **Протокол RU-сессий** (`plans/ru-track-nicole.md` § Session protocol — additions 2026-09-08): холодная разминка из очереди открывает сессию и есть единственный замер; ответ = проверка + ответ; каждое правило контрастной парой; контакт не реже раза в 2–3 дня; условия пробника квиза. Регистрация категории: `question-schema.md` (список + таблица префиксов), `question-bank-taxonomy.md` §7, roadmap (банк 2,415 / RU 169, следующий шаг = вердикт пробника).

Q count: 2415 (Δ+15) · Version: v20260908

---

## 2026-09-07 · session — RU-track Nicole: mini-mock B 7/12 (≈11.7/20) + разбор по горячим следам 12/13

- **Сессия 3 плана последней недели** (`diagnostics/nicole_ru_final_week_29-31.md`), проведена 07.09 вместо 31.08 — первая после старта школы, CC печатно, без разминки перед замером. Mini-mock B: 12 вопросов в пропорциях формы A (О8·М2·П1·Т1), протокол mock (по одному, «записала», без обратной связи). **7/12** — baseline 7/20 → mock 1 9/20 → ≈11.7/20. Кластеры О 4/8 · М 1/2 · П 1/1 · Т 1/1. Промахи: серебряный (-ЯН → нн) · **не глубокая, а мелкая — 3-й промах а-контраста в логах** · нездоровится (раздельно) · по весеннему лугу (дефис) · прыжок → «глагол» (переноса с «бег» из mock 1 не было). Взято, что раньше падало: запятая перед «и» в сложном (0/2 в mock 1), съел с ъ, кое с кем, справа.
- **Разбор сразу после**: все 5 промахов с ретестом на новых словах 6/6 (нелепый · полёт — цепочку «что → сущ» написала сама · глиняный · по осеннему парку / по-осеннему · не широкая, а узкая). Три из пяти правил воспроизвела после одного наводящего вопроса — знает, не запускает проверку в холодном режиме; тест «хозяин» забыла полностью, переучен парой. Новый якорь а-контраста «запятая + а = спор → раздельно» тут же дал **гиперкоррекцию на «но»** (небольшой, но уютный → раздельно ❌) → контрастная пара → недолгий, но интересный ✅. Финальный раунд вперемешку 6/7, темп 10–45 с на ответ.
- **Записано**: exercises `mock` 7/12 rich (`players/nicole_ru/exercises/1788801187557`, meta.mock_form B-mini, clusters; streak bumped 07.09, 5 сигналов сфолжено) + `error_correction` 12/13 rich (`…/1788801213372`, retest_outcomes; 1 сигнал). Результаты — в файле финальной недели, секция «Результаты». Инвентарь письма: «дефиз» ×3 (встречи ~12–14; просьба напечатать слово верно — не выполнила), наблюдать «седьмое». Тайминг: 21:23–22:10 по Алматы, ~47 мин (mock ~21 с паузами 3–5 мин на №3/№9/№12, разбор ~22 с паузой 7 мин, финал 3) — паузы = отвлечение, не затык. Полный mock 2 на бумаге (формы B/C) не проводился — открыт по желанию Артёма. Следующая разминка: пара а/но · по-… с хозяином и без · -ЯН/-АН/-ИН · слово-действие.

Q count: 2400 (Δ0) · Version unchanged (v20260821)

---

## 2026-08-30 · session — repo: мердж logs-ветки + уборка веток

- `logs/nicole-math-s16-ru-3008` влита в `main` fast-forward (логи Николь + ru-S5 Эрнеста + беседа Б2), main запушен (34 коммита, `b8f20cf..e32e70f`), ветка удалена локально и на origin.
- Генеральная уборка: 13 устаревших веток удалены после проверки содержимого (`git cherry` + наличие файлов/скиллов в main — всё влито или переписано позже); 3 из них уже не существовали на GitHub — вычищен кэш `fetch --prune`. Оставлены сознательно: `backups` — единственный снапшот игроков от 03.05 (решение Артёма). PDF-ветка репетиторского брифинга удалена — мастера в `diagnostics/`, пересоберём при нужде. Итог: локально `main` + `backups`, на origin только `main`.

Q count: 2400 (Δ0) · Version unchanged (v20260821)

---

## 2026-08-30 · session — Ernest: беседа Б2 «Почему Казахстан ≠ „почти Россия"» проведена

- ~6 обменов, вышел сам на сатурации («нет больше желания пока болтать») — по протоколу финиш коротким, без дожима; хвоста нет, арка успела закрыться: корейцы-загадка (сам привёл узбекскую параллель) → депортации → ашаршылық (сцена «прадед из шести выжил»: его ответ «промолчу, ничего не надо отвечать» → апгрейд «пустое молчание → слушающее», «сожалею» отверг — справедливо) → целина/русификация/полигон → **язык-загадку трёх поколений решил сам, формулировка «потому что могут» — точнее не скажешь** → Желтоксан → 1991 → вывод про «почти как в России».
- Вердикт по договору Б1: «понимаю, [это] если бы я про всё это говорил в первый день — но такого не будет, я бы дождался, пока сам всё узнаю из рассказов». Частичное принятие: механику понял, риск для себя отрицает через «я наблюдатель». Сослался на папу: **«папа говорит, всем пофиг»** — не оспаривалось, примирено рамкой «оба правы: 99% времени действительно пофиг, борта — про оставшийся 1%».
- Температуры: 🙂 депортации + язык-ренессанс (решал с интересом) · 🙂 ашаршылық (выслушал, отвечал по делу) · сатурация после вывода — лимит Б2-жанра у него ≈5 обменов, короче Б1. **Рекомендация: Б3/Б4 не инициировать — ждать его запроса или живой школьной фактуры после 1.09; формат следующих — короче (3–4 обмена) и реактивный, от его материала.** Артёму: его «всем пофиг» в разговоре с Эрнестом работает против бортов — лучше зеркалить рамку «99% пофиг / 1% мины», она честная и не спорит ни с кем.

Q count: 2400 (Δ0) · Version unchanged (v20260821)

---

## 2026-08-27 · session — Ernest: беседа Б1 «Перед первым звонком» проведена

- Формат отработал: зацеп → сценки-декодеры по одной, ~10 обменов, вовлечённость ровная до конца (Тайминг: точные timestamps не снимались, по плотности обменов ≈25–35 мин, затыков не было). Пройдено: линейка + гимн-жест (угадал сам с первой попытки), казахский со сцены, апай/ағай (декодировал «тётя/старшая сестра» из контекста), «дай списать» (бахрейнский ответ «ты должен сам учиться» → перекалиброван в местную физику; СОР-нюанс взял сам: «тут ты должен быть готов уж сам»), «не стучать» (инстинкт уже местный: «в такие вещи я никогда не лезу»; граница «серьёзное — другой канал» проговорена), складчины, пять бортов.
- **Скепсис по Борату** («мне кажется преувеличиваешь») → честная калибровка «своим можно, чужим нельзя» + «Very nice!»-факт; в финальном replay назвал Бората единственным, что удивило. Self-report «не считаю что ты мне сильно помог» при фактически полной вовлечённости — принято без спора; канал держать лёгким, пользу не продавать.
- **Новые факты** (для Артёма): это его **6-я школа**, не 4-я · в Бахрейне борд **Edexcel/Pearson, не CAIE** — тревога про смену формата, особенно английский · **моки в декабре** · IGCSE-опции были econ + computer science + psychology. Соц-профиль пересмотреть: «все ко мне дружелюбны, тусовался почти со всеми, лучшие друзья были; сильнее всего в Бахрейне» — историческая адаптация «само и быстро» 5/5; заявленное «одноклассники не будут интересны, ко всем нейтрально» читается скорее как защита после потери бахрейнского круга, чем как мизантропия. Стратегию «пара прикольных + нейтрально» валидировал как его собственную, поправка «тёпло-нейтрально».
- **Парковка** (обещано ему): вытащить из школы спеки CAIE-английского + past papers к декабрьским мокам — на Артёма. Долг закрыт: декодер-шпаргалка `diagnostics/ernest_kk_decoder_cheatsheet.md` создана — передать в телефон. Тизеры следующих: Б2 «почему нельзя сравнивать с Россией — сильная история» · Б4 «счета одноклассников, которых нет у тебя». Выбор оставлен за ним, не дожимать.
...
- **v2 по правкам Артёма**: НВТП снят с повестки — не гражданин РК, с воинским учётом не связан; казахский сужен до рецептивного декодер-слоя — таблица ~30 вкраплений «услышит → значит → метка сам/понимать/табу» вместо производственного минимума; добавлена секция «Исторический слой» — тезисы от ашаршылық и депортаций через Желтоксан и Қаңтар до релокантов-2022 и Бората как ядро бесед; добавлена серия бесед (формат: зацеп → рассказ кусками с чеками → сценки-декодеры по одной → финал; протоколы ru-track, лог в version-log + статус-таблицу, Firestore не трогаем).
- **v3 — продление до Жаңа Қазақстан** (запрос Артёма, факты сверены поиском — часть событий после cutoff): исторический слой расширен до 19 тезисов — референдум-2022 и де-элбасизация; Конституция-2026 (референдум 15.03.2026, 87% за, в силе с 1.07: однопалатный Курултай 145 деп., вице-президент, запрет двойного гражданства, корректировка статуса русского языка, Народный совет вместо АНК); выборы в первый Курултай 23.08.2026 — 5 партий, «Әділет» 71%, за неделю до линейки Эрнеста; возврат активов >1,1 трлн ₸ + «Қазақстан халқына»; экономика олигархов через IGCSE-термины (rent-seeking → political discount → филантропия как license to operate; кейсы: Кулибаевы — «Шубарколь», Halyk-пакет по СМИ; аукционы активов Сатыбалды — «Байсат»); Нацфонд-детям ($130,71 за 2025, у Эрнеста счёта нет — не гражданин); АЭС «Балхаш» (референдум-2024, соглашение с Росатомом 05.2026) + ИИ-ставка как «почему у них ИИ в школе»; предохранитель «оценки в обществе разные — понимать словарь, не занимать сторону». Серия бесед → Б1–Б7: новая Б4 «Жаңа Қазақстан: политика и деньги» (можно в два захода), порядок Б2→Б3→Б4 жёсткий.
- **v4.1 — эконом-микрослой по триггеру** (идея Артёма — сравнение экономик КЗ/РФ/УЗ; вердикт red-team: концептуально по силам IGCSE-эконому, но как отдельная беседа — пятая лекция, режем формат, не тему): три «он сам спросит»-истории — смог (юг без газа + дешёвый экибастузский уголь + котловина/инверсия; мостик к АЭС), цены выше Бахрейна (крупнейший в мире landlocked → логистика в ценах и в марже экспортёров → зарплаты ниже гулфовских), банки+добыча как скелет (производству рядом Китай; заработок на дороге в Китай — Хоргос, Барахолка). Правило: лекцию не запускать; 🔥 на любом → разворот в сравнение трёх экономик рамкой «ты жил во всех трёх» как продолжение Б4.
- **v4 — red-team по перегрузу + рулевой feedback-контур** (запрос Артёма): серия сжата Б1–Б7 → Б1–Б4 + микрослои-5-минутки (словарь года / смолл-ток / декодер школе не конкурент — СОЧ он узнает из чата класса за день; декодер — шпаргалкой в телефон); до школы только Б1 «Перед первым звонком» (день-1 поведение + три правила класса + пять бортов без истории + топ-10 декодера) — иначе с S8–S10 ru-спринта выходило шесть структурных форматов за четыре дня; история/политэкономия — в сентябрь, ≤1/нед. Рулевой контур: после куска выбор «глубже или дальше?» вместо «понятно?», финал-замер «что самое дикое?» + выбор следующей темы им, парковка вопросов (его вопрос бьёт план), карта интереса 🔥🙂💤 в статус-таблице + отчёт Артёму после каждой беседы. Довески: тяжёлый блок всегда закрывается светлым; красные линии = «пять бортов, не минное поле» (аналогия 9/11); Б4 — вход одной винеткой, при 💤 сворачивается без драмы; правило первого приглашения смягчено для интроверта — перенос с конкретикой вместо глухого отказа; для Эрнеста это «разговоры», слово «беседа №» не звучит.
- **v3.1 — сюжеты вместо фамилий** (локальные факты Артёма + верификация): блок из 5 винеток для Б4 с карты города Эрнеста — Мега/Almaty Museum of Arts (Смагулов, открыт 12.09.2025; семейный поход = иллюстрация беседы), Ботсад-сосед (фонд Утемуратова, $15 млн, «подарок Алматы» + Нархоз), утильсбор → государству сразу после Қаңтар (~700 млрд ₸ через частную компанию, связывавшуюся с дочерью первого президента), шахта «Костенко» 46 погибших → ArcelorMittal лишился комбината → Qarmet, тройка ERG → 39,3% проданы Шахмурату Муталипу за $1,4 млрд 22.05.2026 (Ибрагимовы сохранили 20,7%, у государства 40%). Установка в беседе: фамилии — фон, запоминается сюжет.

Q count: 2400 (Δ0) · Version unchanged (v20260821)

---

## 2026-08-28 · session — RU-track Nicole: числительные-разряды + ретесты — 6/8, план последней недели закрыт

- Пришла сама («я вернулась») — блок числительных, последний контентный пункт плана из results mock 1. Детектор «сколько? / который по счёту? / что?»: первый заход «третий» ❌ (количественное), после детектора 4 подряд чисто, включая сущ-ловушку «двойка» и **ретест mock №15 (пятый ✅** — вчера «пять»).
- **Ретесты хвостов**: карманный — НН ✅ (промах разбора 26.08, цепочка «карман+н» сработала) · **по-другому — 3-й промах** (`po_hyphen_adverb_missed` count 3: mock №8 → по-блок → сейчас, слово-резист); новый якорь — компания «по-моему/по-твоему/по-своему/по-другому», немедленный ретест ✅, отложенный — обязателен в разминке перед mock 2.
- **Записано**: exercises `error_correction` 6/8 rich (`players/nicole_ru/exercises/1787928529718`, streak bumped 28.08). ⚠ Поле `date` в строке ошибочно `2026-08-27` (лог-скрипт получил дату из продолжающейся сессии); REST-патч отбит 403 (rules: exercises write-once), Firebase MCP отключён — поправить при следующем подключении MCP или учесть в stats-review. «Дефиз» встреча ~11 (инлайн не давался). Все 4 пункта плана последней недели пройдены; осталось: mock 2 форма B на бумаге 30–31.08 (формы B/C собрать) + разминка-ретест по-другому/ветреный перед ним.

...
## 2026-08-27 · session — Ernest: культурный бриф к казахстанской школе

- По запросу Артёма («какие культурные особенности знать, как влиться в новый коллектив») — `plans/school-adaptation-ernest.md`: рамка «два регистра» Cambridge-поток / госстандарт-блок, иерархия и апай/ағай, негласные правила класса (не стучать · дай списать · складчины), казахский минимум как сигнал уважения, красные линии пост-2022 (сравнения с РФ, Қаңтар, Борат, конина, «вы как узбеки»), СОР/СОЧ-словарь, смолл-ток (Кайрат: ЛЧ-сказка 2025/26 → вылет от «Левски», осенью Лига конференций; Сатпаев → «Челси»; ЧМ-2026 — Испания), интровертная тактика входа (баскет-секция как место силы, правило первого приглашения, метрика «знает имена / зовут» вместо «друзья появились?»), родительский слой + сводные вопросы школе. Строка в `plans/README.md` Active.
- Свежее проверено веб-поиском: СОР/СОЧ с 1.09.2026 отменены только в началке, в старших живут; новые предметы «Конституция» / «Закон и Порядок» / ИИ; телефоны на уроках — закон 2024; каникулы 26.10–1.11. Уточнено у Артёма: Cambridge-поток с английским основным; баскетбол соло + зал, футбол/единоборства — нет; знакомые вне школы есть, в классе никого.
- **v2 по правкам Артёма**: НВТП снят с повестки — не гражданин РК, с воинским учётом не связан; казахский сужен до рецептивного декодер-слоя — таблица ~30 вкраплений «услышит → значит → метка сам/понимать/табу» вместо производственного минимума; добавлена секция «Исторический слой» — 11 тезисов от ашаршылық и депортаций через Желтоксан и Қаңтар до релокантов-2022 и Бората как ядро бесед Б2–Б3; добавлена серия бесед Б1–Б6 (формат: зацеп → рассказ кусками с чеками → сценки-декодеры по одной → финал; протоколы ru-track, лог в version-log + статус-таблицу, Firestore не трогаем; Б2 жёстко перед Б3, окна: Б1–Б3 до 1.09, Б4–Б6 первые две недели).

Q count: 2400 (Δ0) · Version unchanged (v20260821)

---

## 2026-08-27 · session — RU-track Nicole: блок по-…ому/по-…ски — 8/9, три ретеста взяты

- Продолжение плана последней недели (п. 2 из results mock 1), запрошено Артёмом, отвечала Ника. Введён вопрос-детектор: «как?» без хозяина → дефис · «по какому + сущ.» → раздельно · трюк «выкинь слово»; довесок по-…ски. **8/9**: единственный промах — по-другому (сказала раздельно даже с подсказкой; mock №8 повторён), сразу после контрастной пары «по другому маршруту» обе стороны взяла.
- **Ретесты взяты все три**: по-зимнему холодно ✅ (промах 25.08, `po_hyphen_adverb_missed`) · по осеннему небу ✅ (mock №7) · **ветреная — одна Н ✅** (после нн ×2 во вчерашнем разборе — исключение село). Хвост сигнала: `po_hyphen_adverb_missed` живой (по-другому), добить в разминке перед mock 2.
- **Записано**: exercises `error_correction` 8/9 rich (`players/nicole_ru/exercises/1787824514939`, streak bumped 27.08; поле date поправлено 26→27 через MCP — лог-скрипт получил вчерашнюю дату из продолжающейся сессии). Инвентарь: «дефиз» ×4 в блоке (встречи 7–10, один инлайн с проверкой «дефиса»).
- **Третий блок («давай ещё») — пунктуация, 7/8**: детектор «посчитай моторы» + однородные «запятые только внутри списка». Единственный промах — гиперкоррекция сразу после правила 1 (запятая при одном моторе: «кошка потянулась и зевнула»), после контрастной пары взяла; дальше 6 подряд чисто. **Оба mock-вопроса пунктуации отыграны**: №18 «ветер стих, и дождь» ✅ · №19 без лишней запятой после «лежали» ✅ — кластер пунктуации 0/2 → потенциально 2/2. Лог `players/nicole_ru/exercises/1787824990268`. Осталось из плана: числительные-разряды · добить по-другому · mock 2 форма B (бумага) 30–31.08.

Q count: 2400 (Δ0) · Version unchanged (v20260821)

---

## 2026-08-26 · session — RU-track Nicole: mock 1 форма A — 9/20 (baseline 7/20)

- **Четвёртый player-initiated приход подряд** («давай сделаем упражнения по русскому») — получила mock 1 форму A по протоколу: по одному вопросу, нейтральное «записала» без обратной связи, счёт в конце. **9/20** против baseline 7/20; цель 16+ не достигнута, разрыв 7 при 5 днях до школы. Кластеры: орфография 6/13 (Δ0) · морфология 2/4 (+2) · пунктуация 0/2 (Δ0) · тропы 1/1 (+1).
- **Ключевые чтения**: Н/НН 0/3 при тренированной W1-теме (ответы инвертированы — алгоритм не вызывается); по-…ому путается в обе стороны (`po_hyphen_adverb_missed` count 2); ъ-регресс (дрил 25.08 ✅ → mock ь). Взято из ранее промахнутого: «кое у кого» ✅ (был count 2), «бег»-сущ ✅, вовсе не ✅; без тренировки взяты пре/при и тропы. Полный разбор + план последней недели: `diagnostics/nicole_ru_mock1_form_a_results.md` (Н/НН re-kickoff → по-…ому контраст → 2 правила пунктуации → числительные; mock 2 форма B на бумаге 30–31.08).
- **Записано**: exercises `mock` 9/20 rich (20 items, 11 сигналов, streak bumped, `players/nicole_ru/exercises/1787747131949`) · инвентарь письма: «дефиз» ×2 в ответах mock (встречи 4–5). Тайминг: per-item не снимался (протокол mock — без пауз CC между вопросами); прошла без пропусков, все 20 отвечены.
- **Разбор сразу после mock (сама попросила «давай сейчас»)** — блок Н/НН заново, кандидат №1 из results: алгоритм «сделано из ___ → стык Н+Н или -АН/-ЯН/-ИН», формат ответа с обязательной цепочкой введён после двух промахов-угадываний (карманный ❌ — её же «карманых» из S7, песчаный ❌). После этого цепочку берёт: мышиная/осенний/кожаная/старинные/туманный/серебряная/деревянный/гусиное ✅, mock-слова кожаная и старинные отыграны. **Не держится исключение «ветреный»** — нн ×2, второй раз сразу после объяснения → в ретест следующей разминки. Итог блока 8/12, лог `error_correction` `players/nicole_ru/exercises/1787749249677`. Инлайн: «незнаю» 4-я встреча (дрил берёт / письмо нет — 🔴 подтверждён). Осталось из плана последней недели: по-…ому контраст · 2 правила пунктуации · числительные · ветреный-ретест.

Q count: 2400 (Δ0) · Version unchanged (v20260821)

---

## 2026-08-25 · session — RU-track Nicole: хвост «Слитно/раздельно/дефис» + наречия слитно из письма, 15/19

- **Третий player-initiated приход подряд** (сама: «давай сделаем упражнения по русскому»). 15/19 (79%), fable-5, 0 инцидентов. Хвост темы дефиса закрыт: сложные слова (юго-запад, диван-кровать) и повторы (еле-еле, чуть-чуть) — с одного показа; курьёз — второе вхождение «диван-кровати» назвала раздельно, не узнав то же слово. Ретесты: «по зимнему лесу» (омофон 21.08) взяла сама ✅; обратная сторона «по-зимнему» → сказала слитно ❌ (новый сигнал `po_hyphen_adverb_missed`); «кое о чём» ❌ — повтор «кое с кем» 21.08, `koe_preposition_split` → count 2, кандидат на добивание.
- **Первый дрил кластера «наречия слитно» из harvest-инвентаря** (её же промахи в письме): вдвое ✅, наугад ✅, спереди ❌ (ровно как в письме S11) → после разбора («переди» не живёт отдельно + сзади/сбоку/сверху) ретест сзади/втроём/с крыши 3/3. Производство-ретесты письма: «незнаю» нашла и исправила сама, «объяснить» с ъ с первого раза — оба 🔴-повтора инвентаря в дриле берёт; инлайн поправлены «дефиз» (3-я встреча) и «расдельно» (новая 🟡-строка, приставки на з/с).
- **Записано**: exercises `error_correction` 15/19 rich (19 items, 4 сигнала сфолжены, streak bumped) · observation в coach_notes · инвентарь письма обновлён (4 строки + 1 новая). Тайминг: ~21 мин всего, пауза ~9 мин перед В3 (отвлечение, не затык), активных ~12. Не покрыто: «по-зимнему»-тип (наречие → дефис) добить в следующей разминке вместе с «кое + предлог».

Q count: 2400 (Δ0) · Version unchanged (v20260821)

---

## 2026-08-23 · session — RU S3-retest + S4a Эрнест: односоставные 8/10 (тема закрыта), виды связи начаты

- **~11 мин активных** (открыта 20:49, первый ответ 21:26 — пришёл позже; закончил сам «всё»). По решению Артёма от 22.08: повторный разбор всех 6 промахов S3 + повторный мини-тест по односоставным — **8/10**, вчерашние ловушки (мне/ему + состояние, ед.ч. → безличное, мн.ч. → неопр.-личное, назывное vs неполное) взяты → **S3 закрыта**. Новые промахи: «В лесу тихо» → назывное (слово состояния на -о), «спорили» → опр.-личное (прош. мн.ч.) — дообъяснены.
- **S4a**: виды связи (согласование/управление/примыкание) рассказом + тест «что за слово зависимое?»; практика 6/10 — ядро с первого раза, промахи ровно на трёх исключениях (его/её/их → примыкание; инфинитив → примыкание; шёпотом / у реки → управление).
- Лог: `ernest_ru` kickoff 14/20 (`partial`, meta `S3-retest+S4a`, Тайминг; ⚠ doc `1787503146074` записан с `total/correct` 21/15 — опечатка счёта, doc write-once; истина — `items[]` 20/14, stats-review читать по items), сигналы `sint_odnosostavnye` count→3, `sint_vidy_svjazi` count→2. Трекер +строка; coach_notes observation. **S4b**: разминка = три исключения связи холодно + 1–2 односоставных; затем дополнение/определение/обстоятельство.
...
- Лог: `ernest_ru` kickoff 9/15 (`partial: true`, meta `sprint_session: S3`, блоки, Тайминг-строка), сигнал `sint_odnosostavnye` count→2 (диагностика №2 + S3). Трекер +строка S3; coach_notes observation. План: S4 «Словосочетание и члены предложения» по расписанию (23.08), Контекст от Артёма постфактум: Эрнест бежал ужинать → 0/3 не диагностичен. Решение Артёма: следующая сессия открывается повторным разбором всех 6 промахов S3 с объяснением заново + полный повторный мини-тест по односоставным, затем S4.

...
- Лог: `ernest_ru` kickoff 14/20 (`partial`, meta `S3-retest+S4a`, Тайминг), сигналы `sint_odnosostavnye` count→3, `sint_vidy_svjazi` count→2. Трекер +строка; coach_notes observation. **S4b**: разминка = три исключения связи холодно + 1–2 односоставных; затем дополнение/определение/обстоятельство.

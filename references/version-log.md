...
- **S4a**: виды связи (согласование/управление/примыкание) рассказом + тест «что за слово зависимое?»; практика 7/11 — ядро с первого раза, промахи ровно на трёх исключениях (его/её/их → примыкание; инфинитив → примыкание; шёпотом / у реки → управление).
- Лог: `ernest_ru` kickoff 15/21 (`partial`, meta `S3-retest+S4a`, Тайминг), сигналы `sint_odnosostavnye` count→3, `sint_vidy_svjazi` count→2. Трекер +строка; coach_notes observation. **S4b**: разминка = три исключения связи холодно + 1–2 односоставных; затем дополнение/определение/обстоятельство.
